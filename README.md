...
Веб-прототип графика смен: закреплённые столбцы, фильтр по отметкам легенды, блок **«Сводка»**, **«Отпуска»** (ВП + ОТ), **архив** прошлого года (только просмотр и выгрузка JSON), экспорт. Без сборки — только статические файлы.
...
- Статические месяцы задаются в **`ARCHIVE_DATABASE`** в `app.js` (ключи `YYYY-M`). В селекторе они в группе **«Архив»**; график **только для чтения** (редактирование и Supabase-push для архива не используются как отдельный поток — данные из репозитория).
- **Выгрузка на компьютер**: для открытого архивного месяца — кнопка **«Экспорт для 1С»** (тот же JSON, что для текущего года). JSON **за весь архивный год** (все месяцы из `ARCHIVE_DATABASE` и локальные overrides по этим ключам) можно скачать из консоли разработчика: **`exportArchiveFullYear()`**.

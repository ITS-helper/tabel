...
- **Редактирование** доступно только для **двух календарных месяцев**: предыдущего и текущего (группа в селекторе «Предыдущий и текущий месяц»). Все остальные выбранные периоды — **только просмотр** (в т.ч. push правок в Supabase для таких месяцев не выполняется из-за блокировок в коде).
- В группе **«Архив {год} (с марта)»** — месяцы с **марта** по декабрь года **`CURRENT_SCHEDULE_YEAR`** в `app.js`, кроме двух «живых» месяцев. Данные берутся из **`DATABASE`**; при отсутствии ключа показывается пустой месяц (каркас без сотрудников).
- Дополнительные статические периоды (например другой год) — в **`ARCHIVE_DATABASE`**; в селекторе группа **«Архив (другие периоды)»**.

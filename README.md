...
- **Выгрузка на компьютер**: для открытого архивного месяца — кнопка **«Экспорт для 1С»** (тот же JSON, что для текущего года). В диалоге **«Архив»** — **«Скачать весь архив за год»** — один JSON со всеми месяцами из `ARCHIVE_DATABASE` (плюс локальные overrides по этим ключам, если были).
...
- Периодически сохраняйте выгрузки графика в репозиторий (`scripts/*.js`) и/или делайте **«Экспорт для 1С»** / **«Скачать весь архив за год»** из UI — это копии данных **на момент экспорта**, не замена серверного бэкапа.

...
- В группе **«Архив»** — месяцы с **марта** по декабрь года **`CURRENT_SCHEDULE_YEAR`** в `app.js`, кроме двух «живых» месяцев, плюс ключи из **`ARCHIVE_DATABASE`** (например другой год). Данные берутся из **`DATABASE`** / **`ARCHIVE_DATABASE`**; при отсутствии ключа в `DATABASE` для архивного месяца показывается пустой каркас.
- **Выгрузка**: для любого открытого месяца — **«Экспорт для 1С»**. JSON **по всем архивным месяцам** (март–декабрь года без «живых» + ключи `ARCHIVE_DATABASE`) — в консоли: **`exportArchiveFullYear()`**.
